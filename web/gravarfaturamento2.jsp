...
<%
		...
		if(acao.equals("inc")) {
			...
			if(!Util.isNull(procedimento) && !Util.isNull(qtde) && !Util.isNull(valor)) {
				...
				if(!duplicado.equals("OK")) {
					response.sendRedirect("faturamentos.jsp?cod=" + dados[0] + "&inf=7&detalhes=" + duplicado);
				}
				else {
					banco.gravarDados("faturas_itens", dadosProced, campostabelaProced, (String)session.getAttribute("usuario"));
					response.sendRedirect("faturamentos.jsp?cod=" + dados[0] + "&inf=Procedimento%20Inserido&tipo_pagto=" + pagto + "&subitem=" + dadosProced[0]);
				}
			}
			/*************************************************/
			else
				response.sendRedirect("faturamentos.jsp?cod=" + dados[0]); //Inserido com sucesso
		}
		//Se for alteração
		else {
			//Dados a serem inseridos
			...
			if(!Util.isNull(procedimento) && !Util.isNull(qtde) && !Util.isNull(valor)) {
				...
				if(!duplicado.equals("OK")) {
					response.sendRedirect("faturamentos.jsp?cod=" + id + "&inf=7&detalhes=" + duplicado);
				}
				else {
					ret = banco.gravarDados("faturas_itens", dadosProced, campostabelaProced, (String)session.getAttribute("usuario"));
					response.sendRedirect("faturamentos.jsp?cod=" + id + "&inf=Procedimento%20Inserido&tipo_pagto=" + pagto + "&subitem=" + dadosProced[0]);
				}
			}
			/*************************************************/
			else
				response.sendRedirect("faturamentos.jsp?cod=" + id + "&inf=5");
		}
	...
	//Se for exclusão
		...
		if(ret.equals("OK")) {
			...
			response.sendRedirect("faturamentos.jsp?inf=3"); //Removido com sucesso
		} else
			response.sendRedirect("faturamentos.jsp?inf=" + ret); //Erro na remoção
	}
%>
